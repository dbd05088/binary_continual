...
class HardBinaryConv(nn.Module):
    def __init__(self, in_chn, out_chn, kernel_size=3, stride=1, padding=1):
        ...
        # requires_grad = True는 back propagation이 자동으로 계산되도록 특성을 설정해주는 것
        self.weights = nn.Parameter(torch.rand((self.number_of_weights,1)) * 0.001, requires_grad=True)

    def forward(self, x):
        real_weights = self.weights.view(self.shape)
        # real weights들의 abs 한 후 평균을 취해준 것이 scaling factor (binary만 하면 real network에 비해서 성능이 크게 뒤쳐지기 때문)
        scaling_factor = torch.mean(torch.mean(torch.mean(abs(real_weights),dim=3,keepdim=True),dim=2,keepdim=True),dim=1,keepdim=True)
        #print(scaling_factor, flush=True)
        scaling_factor = scaling_factor.detach()
        # scaling factor와 binary 값을 곱해줌
        binary_weights_no_grad = scaling_factor * torch.sign(real_weights)
        # -1.0보다 작으면 -1.0으로, 1.0보다 크면 1.0으로 값을 고정해주는 것이 clamp 함수
        cliped_weights = torch.clamp(real_weights, -1.0, 1.0)
        # binary weights도 추적 가능하도록 cliped_weight를 더해줌으로써 이제 grad 추적 대상에 포함 (중요 스킬)
