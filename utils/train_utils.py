...
def select_optimizer(opt_name, lr, model, binary):
    if binary:
        if opt_name == "adam":
        ...
        opt = optim.Adam(
                ...
                {'params' : weight_parameters, 'weight_decay' : 0}],
                lr=lr)
    return opt
...
def select_model(model_name, dataset, binary, num_classes=None):
    opt = edict(
        {
            "depth": 18,
